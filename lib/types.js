...
function array_jsonSchema(name) {
  ...
  itemName = name[name.length-1].toLowerCase() == "s" ? name.substr(0, name.length-1) : name;
...
function __describe(name, type) {
  ...
  if (type instanceof Array) {
    ...
    var t = {
      type: "array",
      items: __describe(name && (name[name.length-1].toLowerCase() == "s" ? name.substr(0, name.length-1) : name), type[0])
    }
